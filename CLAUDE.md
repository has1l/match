...
└── js/
    ...
    ├── footballsim.js      # Football движок симуляции — 22 игрока, arrive-steering, физика мяча
...
```
i18n.js → data.js → sdk.js → events.js → game.js → cs2sim.js → footballsim.js → ui.js → main.js
```
...
| `FootballSim` | `footballsim.js` | Движок реального времени — 22 игрока, arrive-steering + физика мяча |
...
|--------|-----------|--------|---------|-------|-------------|------------------|
| Футбол | `football` | 12 | 12 | Да | 2.5 голов | FootballSim — 22 игрока, arrive-steering, физика мяча |
...
## Football Simulation Engine (`footballsim.js`)

Движок реального времени для визуализации 22 игроков на футбольном поле.

### Как работает

- 22 игрока (11 Team A + 11 Team B) в расстановке 4-3-3
- Team A атакует слева направо, Team B — справа налево (зеркало)
- Движение на `requestAnimationFrame` с **arrive-steering**: ускорение от старта, торможение у цели
- У каждого игрока есть `vx, vy` (скорость, % поля/сек) — инерция, нет мгновенных разворотов
- Мяч движется по ease in-out интерполяции между позициями игроков
- Цели (`tx, ty`) обновляются только по событиям (пас, получение мяча), не каждый кадр

### Расстановка 4-3-3 (Team A, индексы 0-10)
...
Team B — те же индексы + 11 (i+11), позиции зеркалятся по x (100 - x).

### Поведение игроков

- **GK**: заблокирован по x на линии ворот, двигается только по y
- **Владелец мяча**: небольшой случайный дриблинг (wander каждые 0.6–1.1 сек), не замерзает
- **Получатель паса**: спринт прямо к точке назначения мяча (`tx = btx, ty = bty`)
- **2 ближайших защитника**: при каждом пасе получают цель на 35% к точке приёма мяча
- **Атакующие игроки позади мяча**: смещаются вперёд на 4% при каждом пасе
- **Все остальные**: возвращаются к формации

### Пас-цепочки (idle, 12 паттернов)

Индексы — позиции в расстановке (0-10), команда определяется текущим владением:
```
[2,5,6,9]   // CB→DM→CM→ST
...
### Build-up цепочки (перед событием)

```javascript
attack:        [5,9]    // DM→ST
counterattack: [10,9]   // LW→ST
penalty:       [6,9]    // CM→ST
one_on_one:    [8,9]    // RW→ST
corner:        [6,8]    // CM→RW
free_kick:     [5,6]    // DM→CM
header:        [7,10]   // AM→LW
long_shot:     [5,6]    // DM→CM
...
```javascript
FootballSim.init(fieldEl, teamA, teamB)      // создаёт 22 игрока, рендерит точки
FootballSim.start()                           // запускает rAF цикл
FootballSim.stop()                            // останавливает цикл
FootballSim.pause()                           // пауза (для событий)
FootballSim.resume()                          // продолжение после паузы
FootballSim.destroy()                         // полная очистка DOM и состояния
FootballSim.dimAll()                          // затемнить всех (opacity 0.18) при событии
FootballSim.restoreAll()                      // вернуть opacity 1
FootballSim.buildUpTo(type, teamAAttacks, cb) // 2-пас build-up → пауза → вызов cb
FootballSim.newRound(isGoal, teamAAttacked)   // сброс к формации, новый цикл
```

### CSS классы

- `.fbp` — базовый стиль игрока (14–18px, круг, border, box-shadow)
- GK: 18px, ST: 17px, остальные: 14px

### Интеграция с UI (`ui.js`)

- `_lvIdlePlayers()` → возвращает `''` для football (FootballSim сам рендерит `.fbp`)
- `_startLiveMatch()` → `FootballSim.init()` + `FootballSim.start()`
- `_lvTriggerEvent()` → `FootballSim.buildUpTo(event.type, event.teamAAttacks, doEventShow)`
- `_handleLiveAnswer()` → `FootballSim.restoreAll()` + `FootballSim.newRound(isGoal, teamAAttacks)`
- `_showGameOver()` / `_finishLevel()` → `FootballSim.destroy()`
...
- **Футбол**: FootballSim рендерит 22 игрока (`.fbp`), `_lvIdlePlayers()` возвращает `''`, мяч `#lv-ball` контролируется движком; build-up перед событием через `FootballSim.buildUpTo()`
...
- [x] Футбол: движок симуляции (FootballSim) — 22 игрока, arrive-steering, физика мяча, build-up перед событиями
...
- Если меняешь футбольное движение — правь `footballsim.js`.
